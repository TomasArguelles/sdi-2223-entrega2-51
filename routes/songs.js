...
module.exports = function (app, songsRepository, commentsRepository) {
    ...
    app.get('/songs/buy/:id', function (req, res) {
        ...
        songsRepository.userCanBuy(req.session.user, songId, function (bool) {
            if (bool) {
    ...
    app.get('/songs/:id', function (req, res) {
        ...
        songsRepository.findSong(filter, options).then(song => {
            songsRepository.userCanBuy(req.session.user, ObjectId(req.params.id), function (bool) {
                commentsRepository.getComments({song_id: ObjectId(req.params.id)}, options).then(comments => {
                    let settings = {
                    ...
                    rest(settings, function (error, response, body) {
                        ...
                        // nuevo campo "usd" redondeadoa dos decimales
                        let songValue= rateUSD * song.price;
                        song.usd = Math.round(songValue * 100) / 100;
                        res.render("songs/song.twig", {song: song, comments: comments, buy: bool});
                    })
                })
            });
        }).catch(error => {
